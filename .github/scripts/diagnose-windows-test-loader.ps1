...
function Invoke-WindowsTestLoaderDiagnostics {
    param(
        [string]$BinaryPath,
    ...
    $installations = & $InvokeTool $vswhere @('-all', '-prerelease', '-products', '*', '-format', 'json') | ConvertFrom-Json
    $dumpbin = $null
    ...
    if ($null -ne $dumpbin) {
        ...
        & $InvokeTool $dumpbin @('/imports', $resolvedTestBinary.FullName)
        Write-Output "test-loader-imports-exit=$LASTEXITCODE"
        & $InvokeTool $dumpbin @('/headers', $resolvedTestBinary.FullName)
...
if ($MyInvocation.InvocationName -ne '.') {
    Invoke-WindowsTestLoaderDiagnostics -BinaryPath $TestBinary
}
