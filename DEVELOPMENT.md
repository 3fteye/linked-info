...
- `ContentMarkerRegistry` 为每种扩展登记标识、选区动作、校验、画布/列表展示、受控动作和语义披露策略。敏感扩展的 payload 在进入智能引用的向量、持久缓存和复核 LLM 前剥离；其他模型流程必须分别建立同等边界测试。加密工作区保存和客户端密文备份仍保留原始标记与 payload。
