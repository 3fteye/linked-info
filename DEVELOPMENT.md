...
- 内容使用方式可扩展：Markdown 通过基础内容处理器加入，TOTP 通过可叠加的行级增强器加入，脚本能力必须经过受控执行边界；这些用途不固化为节点类型，也不允许 WebView 任意执行节点文本。
...
flowchart LR
    ...
    CP -.-> BUILTIN["内置 Markdown 处理器 / TOTP 行级增强 / 受控脚本边界"]
...
- TOTP 作为行级内容增强而不是独占整个节点的处理器。只有位于 Markdown 代码块之外、整行符合 `TOTP: <Base32>`、`TOTP：<Base32>` 或 `TOTP: <otpauth://totp/...>` 的内容才会启用；前缀大小写不敏感，Base32 分组空白和大小写只在计算时规范化，存储原文不改。未加前缀的相似字符串不猜测、不提示。
- TOTP 只在本机内存计算当前验证码；列表视图只显示遮罩占位，画布展示验证码、剩余时间与受控复制动作。密钥行和当前验证码不得进入日志、向量缓存或智能引用的嵌入/复核 LLM 输入；加密工作区保存和客户端密文备份仍保留原始行。其他模型流程接入增强器正文前必须分别建立同等边界测试，不能根据这条规则假定已经自动覆盖。
...
源码已经建立共享 `ContentProcessorRegistry`、行级 `ContentEnhancerRegistry` 与 `NodeContentHost`。画布和列表统一通过宿主展示正文；节点级基础格式当前内置纯文本和 Markdown，TOTP 等局部用途由行级增强叠加，避免一个节点只能选择一种用途。用户在节点编辑器中显式选择基础格式，默认纯文本，选择作为工作区视图元数据参与保存与历史。Markdown 使用 CommonMark 渲染并忽略原始 HTML；链接降级为不可导航文字，图片降级为占位文字，因此节点预览不会加载远端资源。未知处理器标识会降级为纯文本展示并原样保留选择。节点编辑状态仍由节点卡片局部持有，DOM 回归测试覆盖长文本中间输入的光标、Enter、Shift+Enter、Backspace、处理器切换和节点内部焦点切换；真实 Edge 回归覆盖选择、失焦保存与重载展示。
