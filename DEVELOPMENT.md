...
源码已经建立共享 `ContentProcessorRegistry` 与 `NodeContentHost`，画布和列表统一通过宿主展示正文。当前内置纯文本和 Markdown；用户在节点编辑器中显式选择，默认纯文本，选择作为工作区视图元数据参与保存与历史。Markdown 使用 CommonMark 渲染并忽略原始 HTML；链接降级为不可导航文字，图片降级为占位文字，因此节点预览不会加载远端资源。未知处理器标识会降级为纯文本展示并原样保留选择。节点编辑状态仍由节点卡片局部持有，DOM 回归测试覆盖长文本中间输入的光标、Enter、Shift+Enter、Backspace、处理器切换和节点内部焦点切换；真实 Edge 回归覆盖选择、失焦保存与重载展示。
