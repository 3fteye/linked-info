...
设备本地自动备份历史由独立的 `WorkspaceBackupHistory` 前端端口和 Rust 文件适配器提供。Rust 只在主工作区原子写入成功后复制完整主文件，按一小时最小间隔、30 份和 512 MiB 双上限轮换；历史读取在 Rust 边界完成密文认证，再交给前端共用的工作区验证器。启用加密时，既有历史先写入独立待提交目录并逐份验证，之后才与主文件和 vault 元数据一起提交，因此不会在已经宣称加密后留下旧的明文快照。自动历史、导入前恢复副本和可移植导出采用不同接口与生命周期，不能互相替代。
