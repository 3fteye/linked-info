...
解锁后的授权仍属于 Rust 安全状态，不等于 React 界面可无限期读取明文。安全加固按三层边界实施：CSP 限制 WebView 可以加载和连接的来源，Tauri capability 限制窗口能调用的插件及 IPC，Rust 命令再校验工作区是否解锁、敏感操作是否刚完成重新认证以及异步任务代次是否仍有效。扩展请求只在短暂的“代数复验—秘密剥离—IPC 写入”门控内与锁定排序，绝不持有门控等待不可信来宾响应；锁定先推进 Rust 与扩展运行时的撤销边界，再停止模型、清理令牌和卸载界面。任一清理失败都不能恢复授权。
