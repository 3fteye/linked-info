...
- 桌面端已经提供受管理 `.liext` 安装、逐项授权、启停、升级、卸载保留和显式清除元数据入口；包经过重新验证后进入内容寻址的应用数据目录，不能从任意磁盘位置或网络自动发现 JavaScript、Wasm 或原生库。Schema 升级会在独立 Wasmtime 子进程中迁移工作区级和逐节点 payload，节点 ID 不进入扩展调用；全部结果通过新 Schema 和容量边界后才提交，普通失败会回写旧元数据。已启用且复验有效的第三方处理器和动作会按包内本地化资源动态注册到共享展示宿主，并在同一隔离子进程边界执行；当前仍缺稳定第三方 SDK 和仓库外真实扩展的完整验收，因此不能把这一阶段宣传为开放框架已经完成。
