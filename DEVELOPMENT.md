...
已配置目标通过 `update_s3_backup_target` 原地修改，不能用“删除后重建”代替密钥轮换。命令必须持有目标级操作锁并消费一次性敏感操作授权；候选配置先用新的或现存凭据完成远端 `ListObjectsV2` 验证，验证成功后才把新凭据写入系统安全存储并原子替换带 HMAC 的目标配置。配置文件与系统密钥环之间使用带 HMAC 的持久化事务日志协调创建、更新和移除；正式配置提交后旧凭据清理失败只返回明确的待重试状态，后续读取会继续协调，不能覆盖未完成日志。失败时保留原配置；只轮换凭据或改名时保留目标 ID、创建时间及上传、校验、恢复演练状态；端点、区域、桶或前缀变化时必须清空所有远端派生状态、将自动备份标记为待上传并关闭自动保留清理。
