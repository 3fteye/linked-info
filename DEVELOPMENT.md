...
便签归档是渐进重构的首个切片。第二个切片 `workspaceRecoveryOperation.ts` 协调已准入的恢复撤销/重做交换，明确返回 `committed / notCommitted / recoveryRequired / ownerExpired`；App 仍负责共享门禁、历史方向和界面发布。存储端拒绝只表示提交前失败，已提交但不可读时必须返回 `reloadRequired`。磁盘已提交后的界面发布异常同样进入恢复屏障，不能当作可重试的交换失败；过期 owner 的迟到成功或失败不恢复编辑或便签接纳。首次恢复确认、扩展提案和加密事务暂保留原有协调路径，不宣称所有操作已统一。工作区 v6、Rust 提交点、授权校验及跨文件恢复协议均不改变。
