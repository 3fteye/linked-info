...
- 当前三个注册表仍是源码级静态扩展点，不是动态插件系统。新增内置处理器需要在仓库内实现接口、登记双语名称和能力、补齐安全/兼容性/性能测试并重新构建。工作区 v3 已具备通用、带版本和容量限制的扩展元数据命名空间，但尚无第三方扩展安装、授权或运行时，因而没有任何外部代码能够读写该容器。应用现在不会从磁盘或网络发现并加载第三方 JavaScript、Wasm 或原生库，也没有承诺稳定的第三方 SDK。
...
源码已经建立共享 `ContentProcessorRegistry`、`ContentMarkerRegistry`、兼容行增强器、声明式 `ExtensionPresentationHost` 与 `NodeContentHost`。它们已经足以让仓库贡献者在不改变核心数据模型的前提下增加新的内置展示与局部处理能力，但尚未形成让外部作者独立发布和安装扩展包的开放插件框架。画布和列表统一通过宿主展示正文；节点级基础格式当前内置纯文本、Markdown、九种语言/格式的代码预览和走扩展宿主的 JSON 检查器，局部用途由内容标记叠加，避免一个节点只能选择一种用途。用户在节点编辑器中显式选择基础格式，默认纯文本，选择作为工作区视图元数据参与保存与历史。Markdown 使用 CommonMark 渲染并忽略原始 HTML；链接降级为不可导航文字，图片降级为占位文字，因此节点预览不会加载远端资源。代码高亮使用懒加载的结构化 token 渲染，不执行正文。未知处理器或内容标记会安全降级并原样保留。节点编辑状态仍由节点卡片局部持有，DOM 回归测试覆盖长文本中间输入的光标、Enter、Shift+Enter、Backspace、处理器切换、选区标记和节点内部焦点切换；真实 Edge 回归覆盖选择、失焦保存与重载展示。
