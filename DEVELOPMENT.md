...
杂乱文档采用独立的增量导入草稿，不通过完整工作区备份格式覆盖现有数据。TXT、Markdown 或粘贴文本先在本机确定性分段，再由当前本地 GGUF 模型逐段返回受 JSON Schema 限制的候选节点和引用名称；TypeScript 负责跨段合并、精确名称匹配、来源追踪、草稿编辑与目标快照生成。模型输出只有通过人工选择和画布差异预览后，才能作为一个工作区历史事务保存。
