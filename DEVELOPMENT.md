...
- `view.extensionMetadata` 自工作区 v3 起按扩展 ID 隔离，v4 继续保留；扩展记录具有独立 `schemaVersion`、工作区级对象和按节点 ID 索引的对象。外层节点 ID 必须存在，未知扩展的数据仍被保留。
...
开放扩展框架的包格式、能力模型、元数据、隔离运行时、实施顺序和完成标准集中在 [docs/extension-framework.md](docs/extension-framework.md)。本节只描述当前已经落地的内置内容处理边界；不能根据本节现有注册表推断第三方安装能力已经完成。
...
- 将来只有在扩展清单与 API 版本、只读节点快照、按扩展 ID 隔离的元数据、未知扩展保留、迁移与容量限制、受验证写操作以及隔离加载全部具备后，才允许对外称为开放扩展框架。核心领域层不会提供注册新节点字段、替换引用语义或绕过不变量的扩展点。
...
当前工作区逻辑格式为 v4：节点和引用仍属于整个工作区；画布集合、当前画布、各画布布局与视口统一位于 `view`，不进入 `InformationNode` 或 `Reference`。同一节点可以出现在多个画布，也允许没有位置；单个画布内同一节点最多一个位置。引用只在两个端点都位于当前画布时绘制，但筛选、导出和持久化继续使用全部正式引用。v1～v3 的顶层布局与视口确定性迁移到一个默认画布，之后不再在内部同时维护旧顶层字段和新画布结构。TypeScript 和 Rust 共同读取 `fixtures/workspace-contract.json`，对合法多画布、旧版本迁移、重复名称、悬空引用、布局、视口、处理器选择和扩展元数据执行同一组边界测试。
