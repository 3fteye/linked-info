...
- 外部 Wasm 的执行代码只存在于独立 `linked-info-extension-host` 子进程。桌面 Rust 只依赖轻量 IPC 契约，不链接 Wasmtime；子进程重新验证 `.liext` 包，使用空 Linker 只允许无运行时能力的 WIT 类型导入，拒绝函数、模块、实例、资源及通用 WASI，并在每次调用的新 Store 上执行能力、输入、输出、128 MiB 线性内存、fuel、epoch 和墙钟限制，进程本身另受 512 MiB 总内存上限。子进程安装进程上限并完成 Hello 后才读取不可信包。IPC 前由桌面 Rust 再次剥离已知 `secret`、`totp` 与旧式 TOTP payload；锁定、休眠、工作区替换、加密启用和数据密钥轮换通过独立终止句柄立即杀死所有宿主，不能等待扩展响应或与普通调用争用同一把锁。
...
源码已经建立共享 `ContentProcessorRegistry`、`ContentMarkerRegistry`、兼容行增强器、声明式 `ExtensionPresentationHost`、`NodeContentHost` 和隔离 Wasm 宿主底座。它们已经足以验证内置适配及不可信组件的执行边界，但在安装生命周期、用户授权、修改提案接入和正式 SDK 完成前，仍不是外部作者可以实际安装的开放插件框架。画布和列表统一通过宿主展示正文；节点级基础格式当前内置纯文本、Markdown、九种语言/格式的代码预览和走扩展宿主的 JSON 检查器，局部用途由内容标记叠加，避免一个节点只能选择一种用途。用户在节点编辑器中显式选择基础格式，默认纯文本，选择作为工作区视图元数据参与保存与历史。Markdown 使用 CommonMark 渲染并忽略原始 HTML；链接降级为不可导航文字，图片降级为占位文字，因此节点预览不会加载远端资源。代码高亮使用懒加载的结构化 token 渲染，不执行正文。未知处理器或内容标记会安全降级并原样保留。节点编辑状态仍由节点卡片局部持有，DOM 回归测试覆盖长文本中间输入的光标、Enter、Shift+Enter、Backspace、处理器切换、选区标记和节点内部焦点切换；真实 Edge 回归覆盖选择、失焦保存与重载展示。
