...
- 当前三个前端注册表仍是源码级静态扩展点，尚未成为动态第三方注册表。桌面端已经提供受管理 `.liext` 安装、逐项授权、启停、升级、卸载保留和显式清除元数据入口；包经过重新验证后进入内容寻址的应用数据目录，不能从任意磁盘位置或网络自动发现 JavaScript、Wasm 或原生库。Schema 升级会在独立 Wasmtime 子进程中迁移工作区级和逐节点 payload，节点 ID 不进入扩展调用；全部结果通过新 Schema 和容量边界后才提交，普通失败会回写旧元数据。当前仍未把已启用第三方包的处理器和动作动态接入共享展示宿主，也没有稳定第三方 SDK，因此安装入口本身不能被宣传为开放框架已经完成。
...
源码已经建立共享 `ContentProcessorRegistry`、`ContentMarkerRegistry`、兼容行增强器、声明式 `ExtensionPresentationHost`、`NodeContentHost`、隔离 Wasm 宿主、修改提案闭环和受管理安装目录。它们已经足以验证内置适配、不可信组件执行边界以及“预览—复验—单事务保存”；第三方包也可以完成受控安装生命周期，但在动态贡献接入、崩溃中断的升级恢复日志和正式 SDK 完成前，仍不是外部作者可以走通完整使用闭环的开放插件框架。画布和列表统一通过宿主展示正文；节点级基础格式当前内置纯文本、Markdown、九种语言/格式的代码预览和走扩展宿主的 JSON 检查器，局部用途由内容标记叠加，避免一个节点只能选择一种用途。用户在节点编辑器中显式选择基础格式，默认纯文本，选择作为工作区视图元数据参与保存与历史。Markdown 使用 CommonMark 渲染并忽略原始 HTML；链接降级为不可导航文字，图片降级为占位文字，因此节点预览不会加载远端资源。代码高亮使用懒加载的结构化 token 渲染，不执行正文。未知处理器或内容标记会安全降级并原样保留。节点编辑状态仍由节点卡片局部持有，DOM 回归测试覆盖长文本中间输入的光标、Enter、Shift+Enter、Backspace、处理器切换、选区标记和节点内部焦点切换；真实 Edge 回归覆盖选择、失焦保存与重载展示。
