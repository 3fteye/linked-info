...
正式构建不加载远端脚本，远端备份和 AI 请求由 Rust 适配器执行。文件导入导出将从 WebView 的通用文件读写插件权限迁移为一次性的 Rust 命令。开发服务器需要的热更新与调试来源只能进入开发配置，不能放宽生产 CSP。完整规则见 [SECURITY.md](SECURITY.md)。
