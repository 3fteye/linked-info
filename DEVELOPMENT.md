...
- 外部 Wasm 的执行代码只存在于独立 `linked-info-extension-host` 子进程。桌面 Rust 只依赖轻量 IPC 契约，不链接 Wasmtime；子进程重新验证 `.liext` 包，使用空 Linker 拒绝全部宿主导入和通用 WASI，并在每次调用的新 Store 上执行能力、输入、输出、128 MiB 线性内存、fuel、epoch 和墙钟限制，进程本身另受 512 MiB 总内存上限。子进程安装进程上限并完成 Hello 后才读取不可信包。IPC 前由桌面 Rust 再次剥离已知 `secret`、`totp` 与旧式 TOTP payload；锁定、休眠、工作区替换、加密启用和数据密钥轮换通过独立终止句柄立即杀死所有宿主，不能等待扩展响应或与普通调用争用同一把锁。
