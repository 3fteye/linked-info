...
- 当前 `ContentProcessor` 只接收节点的可空正文字符串并返回受约束的展示模型；它不接收名称、引用、布局或通用节点元数据，也没有正式数据写入接口。处理器选择属于独立视图元数据，不进入领域层。
- 当前三个注册表是源码级静态扩展点，不是动态插件系统。新增内置处理器需要在仓库内实现接口、登记双语名称和能力、补齐安全/兼容性/性能测试并重新构建；工作区也只有固定的 `contentProcessorByNodeId` 选择表，没有供扩展自由定义的节点元数据命名空间。应用现在不会从磁盘或网络发现并加载第三方 JavaScript、Wasm 或原生库，也没有承诺稳定的第三方 SDK。
