...
节点模板通过宿主模块 `nodeTemplates.ts` 保存到现有 v6 工作区的 `view.extensionMetadata["app.linked-info.node-templates"]`，模板库内部 `schemaVersion: 1`，`workspace.templates` 为定义数组，`byNodeId` 为空。定义只保存 ID、模板名、经过秘密清空的正文和目标节点 ID 创建指令；它们不是已成立的 `Reference`。创建时才复验目标并原子添加普通节点、当前画布位置及正式引用。工作区版本不变，旧版会保留未知元数据；新模块拒绝未知模板版本或损坏内容，不静默覆盖。沿用容器的 64 KiB 总量限制，另限制 128 份模板、每份 4096 正文字符和 128 个目标；超限明确报错而不截断。

模板准备使用既有内容标记解析器，清空秘密/TOTP 载荷并保留注释，兼容旧式 TOTP 行。无法解释的标记拒绝处理；未标记的秘密及注释仍要求用户预览检查。模板弹窗关闭或会话锁定后不保留额外本地草稿，模板与工作区共用保存、加密、备份、恢复和数据撤销路径。

`canvasNavigation.ts` 提供最多 100 个位置的会话内返回/前进栈，App 在显式切换画布、搜索聚焦、引用筛选和书签跳转前记录位置与筛选上下文。普通视口变化不逐帧入栈，回放不进入数据历史；工作区锁定和替换清除导航栈与待定位请求。快捷键 `Alt+← / Alt+→` 不接管输入框、按钮、编辑节点和对话框的原生操作。测试、审查和交付状态以当前功能 PR 为准。

### 前端操作协调：便签归档切片

第三切片 `workspaceReplacementOperation.ts` 负责首次恢复确认：普通快照先保留恢复副本再保存目标，加密初始化恢复先排空旧保存再调用 Rust 原子提交并回读权威快照。两条路径不共用错误猜测：普通 `save(): Promise<void>` 调用后的异常无法证明未提交，因此保持恢复屏障；原子安全事务按既有结构化结果处理。界面发布异常与 owner 过期都不重新开放写入，App 在协调返回后再复验 owner。确认界面及工作区 v6 不变；执行中统一暂停旧快照自动保存，明确的提交前失败才恢复编辑。

窗口关闭拦截器跟随 App 生命周期注册，不随 `persistenceReady` 加载状态卸载。首次恢复或恢复交换在途时拒绝关闭，加载界面显示失败提示；事务结束后可重试关闭。无在途替换的加载/错误界面不写入旧快照即可退出，既有便签提交仍由关闭流程等待。加载状态不再承担原生生命周期注册职责。

`workspaceArchiveOperation.ts` 协调一次便签领取、准入复验、候选准备、旧保存排空、提交与权威快照回读，不依赖 React 或 Tauri。`App` 通过当前工作区的同步租约接入共享 mutation gate、最后快照候选、历史与界面状态；队列调度仍由原调用点负责，不建立第二套写入队列或安全授权。

归档以 `preparing / committing / committed` 区分错误所处阶段。只有提交端明确返回 `capsule_commit_not_saved` 才能在调用提交后按未保存处理；提交已完成、结果未知或回读失败均保留恢复屏障。owner 过期时不发布迟到结果，不重新开放编辑。只有实际取得租约的操作才清理自己的候选，未准入的归档不能清掉其他操作的状态。模块已纳入敏感日志检查；测试使用合成数据，执行验证仅交给 CI。

便签归档是渐进重构的首个切片。第二个切片 `workspaceRecoveryOperation.ts` 协调已准入的恢复撤销/重做交换，明确返回 `committed / notCommitted / recoveryRequired / ownerExpired`；App 仍负责共享门禁、历史方向和界面发布。存储端拒绝只表示提交前失败，已提交但不可读时必须返回 `reloadRequired`。磁盘已提交后的界面发布异常同样进入恢复屏障，不能当作可重试的交换失败；过期 owner 的迟到成功或失败不恢复编辑或便签接纳。首次恢复确认见第三切片；扩展提案和其他加密事务暂保留原有协调路径，不宣称所有操作已统一。工作区 v6、Rust 提交点、授权校验及跨文件恢复协议均不改变。
...
自动排版另有独立的 Windows 冷启动回归任务：`node scripts/test-arrangement-cold-start.mjs` 默认只检查实际修复配置，使用全新 Vite 缓存、关闭重试，必须完整通过且没有优化器刷新或意外导航。常规完整 Edge 套件继续保留，不要求历史时序缺陷每次重现才能检查当前代码。

历史负对照单独保留为 `node scripts/test-arrangement-cold-start.mjs --experiment`；通过 CI 的 `workflow_dispatch` 勾选 `cold_start_experiment` 执行，不在用户机器启动。只有同时取得依赖优化器发现 `@dagrejs/dagre`、优化器请求整页刷新及测试导航断言失败的证据才算复现；其他错误不能充当根因证明。负对照未证明时仍执行修复组，分别报告结果，但实验整体保持失败，不能把“未证明”算成通过。未知参数在执行前拒绝；判定和协调有独立 Node 回归。本轮结果以新的 CI 为准，不能把编写了回归等同于验证通过。

每次唯一缓存目录位于 runner checkout 的 `apps/desktop/node_modules` 下，保持 React 插件对优化产物的默认排除语义。不能改放系统 Temp：此前夹具这样做时，Babel 重新转译了优化后的 `react-dom_client.js`，引入额外负载；严格判定器会拒绝普通弹窗超时等非导航失败，而不把它算成成功复现。实验两组仍使用不同的新缓存，路径契约回归不启动浏览器或写入文件。CI 33868683243 的负对照仍出现非导航弹窗超时并阻断了修复组，说明负对照不应作为常规门禁的前置条件；不能把这次失败归因给已移除的 Temp 缓存路径。
...
这些小请求只能定位连接和协议层差异；即使都返回 200，也不能代替正式生产依赖审计。Node 原生 fetch 也不等于 pnpm 内置客户端。CI 33868683243 的同 runner 诊断已确认 DNS/TCP/TLS 和 ping 可用，小型未压缩 bulk 请求出现等待、gzip 请求本次较快；六个顺序样本不足以把压缩或 pnpm 本身认定为根因。诊断代码由 fake transport 回归覆盖，验证只在 CI 进行。

下一步有界实验仍用 pnpm 11.16.0、同一锁文件、官方 registry 和原生产依赖高危审计命令，只给该步骤设置 `PNPM_CONFIG_FETCH_TIMEOUT=180000`、`PNPM_CONFIG_FETCH_RETRIES=0`，步骤硬上限四分钟。这是单请求最多 180 秒、零重试，少于原约 251 秒总等待，不更换审计器、不截取真实请求体、不修改忽略规则或锁文件。pnpm 11 的配置入口是 `PNPM_CONFIG_*`，不是旧 `npm_config_*`。步骤耗时包含进程启动和依赖解析；若小于原 60 秒预算即成功，只能记录本次审计完成，不能归功于延长单请求预算；仍失败则保留失败，不继续加预算。
