...
悬浮胶囊切片已写入实现，当前改动尚未运行 CI，也未完成原生 Windows 实机验收。主窗口通过 Rust 签发的 `ownerId` 独占正式工作区快照、保存队列与历史；读、写和恢复交换必须携带原 owner，不能由旧 React 实例重新取得 permit。胶囊是独立入口和独立窗口，只持有当前有界草稿，不加载整个工作区。Rust 同时核对真实窗口与 WebView label，以专属白名单隔离胶囊和主窗口命令。
