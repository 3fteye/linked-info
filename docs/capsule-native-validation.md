...
2026-09-03，[Windows package 33735471628](https://github.com/3fteye/linked-info/actions/runs/33735471628) 对运行时代码 `244ddff` 的正式 EXE 完成 14 项原生检查，14 passed、0 failed、0 page errors，测试进程正常退出码为 0。覆盖真实窗口拖动、原生失焦提交、重复提交去重、单次撤销、主窗口敏感命令拒绝、加密与最后快照锁定、通知撤权、隐藏和统一退出。CI 只保存结构化 JSON 摘要及正常 Windows 包，不包含测试工作区或浏览器目录。
